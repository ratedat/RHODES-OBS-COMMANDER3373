...
大会入力を開始した後の保存済みラン状態は、再起動時にも引き継ぎます。接続は自動再開しないため、状態を確認して大会入力を開始し直し、新しい入力URLを担当者へ渡してください。「ランをクリア」で引き継ぎを解除すると、次の起動は通常どおり空のランから始まります。詳しくは [大会遠隔入力](tournament-remote-input.md) を参照してください。
...
現在のソースからSuki/Avaloniaアプリをビルドし、成功した場合に起動します。

既存のサーバーを自動で停止する処理はありません。ポートが使用中なら、既に動いているアプリを使うか、起動時に別ポートを選んでください。
...
同じ状態ファイルを使うアプリが既に動いている場合、2つ目の起動は現在のランを初期化せずに終了します。既存のウィンドウを使ってください。別の状態ファイルを使う構成は、それぞれ起動できます。

### 保存状態を読み込めない

Node APIが「保存状態を読み込めません」と返す場合、保存済みの状態は初期値で上書きされずに保持されます。状態ファイルを退避して内容とアクセス権を確認してください。OBSの再読み込みを繰り返しても、壊れた状態ファイルは修復されません。
