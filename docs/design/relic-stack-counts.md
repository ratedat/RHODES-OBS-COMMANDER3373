...
- 提示一覧には漏れがあり得るが、誤検出と不要なOCRを防ぐため、一覧登録済みIDだけをスタック取得のallowlistとする。
- 新しいスタック秘宝を確認した場合は、OCRを許可する前に正規ID・テーマ・上限を一覧へ追加する。
...
- ルール未掲載の秘宝に対するスタック値は保存せず、旧stateに残る値も再読込時に破棄する。

## 認識契約

1. `relicsFull` の秘宝名OCRを現在テーマの秘宝マスターへ一意に解決する。
2. allowlist登録済みの解決済み秘宝だけを対象に、秘宝名ボックスをアンカーとしてカード左下の明色スタックマーカーを狭いROIで確認する。
...
- 未掲載秘宝には、旧stateに値が残っていても編集欄を表示しない。
...
- ルール整合、allowlist外拒否、上限、未検出時保持、所持解除時削除、全Overlay表示、Sidecar未起動表示を小さい回帰試験で固定する。
...
- 一覧未掲載の秘宝にはスタックOCRを発行せず、外部入力や旧stateのスタック値も採用しない。
