...
- Center column: operational run-state workbench. Do not add a new top-level tab for every new field; put new features inside these stable workspaces.
  - Run workspace: retained base run value cards for originium ingots, difficulty, squad, and campaign-specific values such as IS#5 conception, age, IS#4 revelation, and IS#6 coins. Hope, life, shield, and command level are not recognition targets. Include an IS switching/current-run context section.
