...
Electron/Tauri 版は、SukiUI 版が機能を引き継ぐまで検証済み実装として保持する。新規の OCR/ADB 改善は原則 MAAFramework 側に寄せ、Windows OCR や単体 PaddleOCR の自前アダプターは通常ランタイムから削除し、MAA-OCR + 任意 GLM-OCR だけを開発対象にする。
