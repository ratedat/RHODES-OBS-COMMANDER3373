...
Electron/Tauri 版は正規の実行・配布経路から外し、必要な機能だけ SukiUI/Avalonia 版へ移す。新規の OCR/ADB 改善は原則 MAAFramework 側に寄せ、Windows OCR や単体 PaddleOCR の自前アダプターは通常ランタイムから削除し、MAA-OCR + 任意 GLM-OCR だけを開発対象にする。
...
- Rejected: 通常ランタイムから外し、MAA-OCR と任意 GLM-OCR に集約する
