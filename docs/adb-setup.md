...
RHODES OBS COMMANDER3373のADB連携は、エミュレーター画面のスクリーンショット取得、タップ、スワイプをアプリ内のNodeサーバーから実行します。ブラウザでHTMLを直接開いただけではADB操作はできないため、通常は配布exeまたはElectronアプリから起動してください。
