...
3. Suki / MAAFramework取得から以下を試します。
   - `共通値`: 源石錐、等級、分隊、構想、時代
   - `秘宝`: 所持秘宝。3行以下ではスクロールせず、4行目以降がある場合だけスクロールします。
   - `思案`: 所持思案
   - `オペレーター`: 招集済みオペレーター
4. 取得中はMuMuやアプリを触らず、完了まで待ってください。
5. 結果がおかしい場合は、Sukiの認識ワークフローにあるスキャン履歴と保存ログのパスを確認してください。

## OBS表示

- OBS Browser Sourceには `Overlay` のURLを使います。
- 例: `http://127.0.0.1:<アプリに表示されたポート>/overlay`
- 今回はMAAFramework取得の精度確認が主目的です。OBSレイアウトは今後調整します。

## 不具合報告でほしい情報

- 何を取得しようとしたか: 共通値 / 秘宝 / 思案 / オペレーター / 時代
- 実際のゲーム画面スクリーンショット
- アプリに表示された取得結果
- Suki認識ワークフローのスキャン履歴に出ている保存パス、またはログJSON
- 使用エミュレーターと解像度
