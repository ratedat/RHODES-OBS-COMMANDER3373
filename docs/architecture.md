...
The active desktop app is `apps/rhodes-suki`, built with Avalonia + SukiUI. Electron and Tauri are not active build, launch, or packaging targets. The local web server remains only for the HTTP API and OBS browser-source output; it is not a desktop app fallback.
