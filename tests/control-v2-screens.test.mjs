...
test("retired control screen metadata provides labels without reviving control routes", () => {
  ...
  assert.equal(getControlV2ScreenMeta("operators").detachPath, "/sidecar");
  assert.equal(getControlV2ScreenMeta("sidecar").detachPath, "/sidecar");
  assert.equal(controlV2ScreenOptions.every((item) => !item.detachPath.startsWith("/control-v2")), true);
