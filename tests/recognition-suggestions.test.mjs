...
test("run status, operator, relic, revelation, thought, age, and coin candidates use stable dedupe keys", () => {
  ...
  assert.equal(recognitionCandidateKey({ kind: "coin", campaignId: "is6_sui", fieldId: "activeCoins", coinId: "c1", statusId: "rust", count: 2 }), "coin:is6_sui:activeCoins:c1:rust:2");
});
...
test("recognition candidates dedupe repeated detections but keep different coin slots", () => {
  const candidates = dedupeRecognitionCandidates([
    ...
    { kind: "coin", campaignId: "is6_sui", fieldId: "coins", coinId: "coin-a", statusId: null, count: 1 },
    { kind: "coin", campaignId: "is6_sui", fieldId: "coins", coinId: "coin-a", statusId: "status-a", count: 1 },
    { kind: "coin", campaignId: "is6_sui", fieldId: "coins", coinId: "coin-a", statusId: "status-a", count: 1 },
  ]);
...
test("recognition suggestions append to pendingSuggestions without mutating active run state", () => {
  ...
  const suggestions = buildRecognitionSuggestions([
    ...
    { kind: "coin", campaignId: "is6_sui", fieldId: "coins", coinId: "coin-a", name: "通宝A", statusId: "status-a", count: 2 },
